...
for (int j=0;j<Height;j++)
   ...
   show_bmp(p, y, x,buff,replacePix,&oldstartR,&oldstartC);
    show_bmp(sprites[0].p, sprites[0].y, sprites[0].x,sprites[0].buffer,sprites[0].replacePix,
         &(sprites[0].oldstartRow),&(sprites[0].oldstartCol));
sprites[0].replacePix=1;
          show_bmp(sprites[1].p, sprites[1].y, sprites[1].x,sprites[1].buffer,sprites[1].replacePix,
         &(sprites[1].oldstartRow),&(sprites[1].oldstartCol));
sprites[1].replacePix=1;
          show_bmp(sprites[2].p, sprites[2].y, sprites[2].x,sprites[2].buffer,sprites[2].replacePix,
         &(sprites[2].oldstartRow),&(sprites[2].oldstartCol));
     sprites[2].replacePix=1;
...
//uprintf("rand %d\n",(rand()+1)%5);
     ...
     switch(key){
      ...
      if (move){
	 //innovation, increasing points with each power up
         if (table[y>>4][x>>4] == P){
             black_point(y,x);
             table[y>>4][x>>4] = V;
               uprintf("point\n");
               ...
               uprintf("Score: %d\n", score);
                int score_fake = score;
               int intLength = 0;
               while(score_fake > 0){
                  int dig = score_fake %10;
                  score_fake/=10;
                  intLength++;
               }
               int furtherNum=intLength;
               for(i=0;i<furtherNum+1;i++){
                  unkpchar(score_array[i],0,60+i);
               }
              
               int anotha = score;
               while(anotha > 0){
                  int dig = anotha %10;
                  score_array[intLength]='0'+ dig;
                  intLength--;
                  anotha/=10;
               }
               for(i=0;i<furtherNum+1;i++){
          ...
          if (table[y>>4][x>>4] == W) {
            ...
            uprintf("\nYou picked up a power up. The points are worth more\n");
            vcounter=vcounter+1;
      }
   ...
   */          
               int vertex = sprites[l].pathV[sprites[l].pathpos];
               sprites[l].x = (vertex % Width) * 16;
               sprites[l].y = (vertex / Width) * 16;
                //sprites[0].x = ????
                //sprites[0].y = ???
                sprites[l].pathpos=1;
            ...
            if (sprites[l].pathpos == sprites[l].pathlen)
